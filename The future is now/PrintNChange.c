...
void print_mundo(char arr[MAX][MAX])                            //Esta funcion imprime la matriz en pantalla
{
    ...
    for (countfil=inicio; countfil<tope; ++countfil)
    {
                                                                //Con esto señalamos el segundo elemento de la segunda fila
        for (countcol=inicio; countcol<tope; ++countcol)
        {
...
{
    int countfil, countcol;
    for (countfil=inicio; countfil<tope; ++countfil)            //Menos a la cantidad de columnas totales
    {
        for (countcol=0; countcol<tope; ++countcol)             //Menor a la cantidad de filas totales
        {
